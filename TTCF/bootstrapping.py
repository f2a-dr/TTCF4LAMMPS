...
class Bootstrap():
    ...
    def __init__(self, variable, nResamples, nTrajectories, nGroups, nTimesteps,
        ...
        self.directory = str(variable) + '_global'
        self.variable_OB = []
        self.variable_O = []
        self.variable_B = []
        self.averageBt = []
    ...
    def concatenateTrajectories(self):
        for i in ['OB', 'O', 'B']:
            outFilename = str(self.variable) + '_' + i +'_global.txt'
            inFilenames = glob.glob(str(self.variable) + '_' + i + '_*.dat')
    ...
    def readTrajectories(self):
        filenames = [self.variable + '_OB_global.txt', self.variable + '_O_global.txt', self.variable + '_B_global.txt']
        arrays = [[], [], []]
    ...
    def groupTrajectories(self):
        ...
        # arrays = [self.variable_OB, self.variable_O, self.variable_B]
        # for i in range(len(arrays)):
        #     arrays[i] = arrays[i].reshape((self.nGroups, int(self.nTrajectories/self.nGroups), self.nTimesteps))
        #     arrays[i] = arrays[i].transpose((0, 2, 1))
        print(self.variable_OB.shape)
        print(self.variable_O.shape)
        print(self.variable_B.shape)
    ...
    def averageWithin(self):
        ...
        # arrays = [self.variable_OB, self.variable_O, self.variable_B]
        # for i in range(len(arrays)):
        #     arrays[i] = np.mean(arrays[i], axis=-1)
        print(self.variable_OB.shape)
        print(self.variable_O.shape)
        print(self.variable_B.shape)
    ...
    def confidenceInterval(self):
        ...
        np.savetxt('ci.txt', confInterval)
    ...
    def plotDistribution(self, timestep=-1, format='png'):
        ...
        ax.legend()
        fig.savefig(str(self.variable) + 'averageDistribution.'+ format, dpi=300)
